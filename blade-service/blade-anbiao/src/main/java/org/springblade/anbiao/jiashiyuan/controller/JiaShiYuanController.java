...
//				}
					...
					r.setMsg(deail.getJiashizhenghao() + "该驾驶证号不合法;");
					r.setCode(500);
					r.setSuccess(false);
					return r;
				}
...
//				}
		...
		if (StringUtils.isNotEmpty(jiaShiYuan.getShenfenzhenghao()) && jiaShiYuan.getShenfenzhenghao() != null) {
			if(deail != null && StringUtils.isNotEmpty(deail.getJiashizhenghao())){
				if (IdCardUtil.isValidCard(deail.getShenfenzhenghao()) == true) {
					jiaShiYuan.setShenfenzhenghao(deail.getShenfenzhenghao());
//			if (deail != null) {
...
//			} else {
					jiaShiYuan.setShenfenzhenghao(deail.getShenfenzhenghao());
					//通过身份证获取年龄
					Integer age = IdCardUtil.getAgeByCard(deail.getShenfenzhenghao());
					jiaShiYuan.setNianling(age.toString());
					//通过身份证获取生日日期
					Date chushengshijian = IdCardUtil.getBirthDate(deail.getShenfenzhenghao());
					jiaShiYuan.setChushengshijian(dateFormat2.format(chushengshijian));
					jiaShiYuan.setXingbie(Integer.toString(IdCardUtil.getGender(deail.getShenfenzhenghao())));
//			}
				} else {
					r.setMsg(deail.getShenfenzhenghao() + "该驾驶员身份证号不合法");
					r.setCode(500);
					r.setSuccess(false);
					return r;
				}
...
//			jiaShiYuan.setId(jiaShiYuan2.getId());
			...
			//入职表身份证
			QueryWrapper<AnbiaoJiashiyuanRuzhi> ruzhiQueryWrapper2 = new QueryWrapper<AnbiaoJiashiyuanRuzhi>();
			...
			//驾驶证号
			QueryWrapper<AnbiaoJiashiyuanJiashizheng> jiashizhengQueryWrapper2 = new QueryWrapper<AnbiaoJiashiyuanJiashizheng>();
...
//					}
	...
	public R<List<JiaShiYuan>> getByIdJiaShiYuanList(String deptId,String jiashiyuanleixing) {
		List<JiaShiYuan> detail = iJiaShiYuanService.jiaShiYuanList(deptId,jiashiyuanleixing);
...
//		}
	...
	@GetMapping("/uploadDriverExport")
	@ApiLog("驾驶员统计信息明细-导出")
	@ApiOperation(value = "驾驶员统计信息明细-导出", notes = "传入deptId，jiashiyuanId", position = 22)
	public R uploadDriverExport(HttpServletRequest request, HttpServletResponse response, String deptId, String jiashiyuanId, BladeUser user) throws Exception {
		R rs = new R();
		String deptName = "";
		String uuid = UUID.randomUUID().toString().replace("-", "");
		SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd");
