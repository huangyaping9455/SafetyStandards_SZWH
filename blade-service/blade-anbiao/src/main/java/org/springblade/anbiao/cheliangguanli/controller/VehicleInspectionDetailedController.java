...
public class VehicleInspectionDetailedController extends BladeController {
	...
	public R remove(@ApiParam(value = "主键集合", required = true) @RequestParam String ids) {
		...
		List<VehicleInspectionDetailed> deptBaoxians = new ArrayList<>();
		for(String id:idsss) {
			VehicleInspectionDetailed baoxian = new VehicleInspectionDetailed();
			baoxian.setVidIds(id);
			baoxian.setVitDelete("1");
			baoxian.setVitUpdateTime(LocalDateTime.now());
			deptBaoxians.add(baoxian);
		}
		return R.status(vehicleInspectionDetailedService.updateBatchById(deptBaoxians));
	}
